          ...
          {foreach from = $nominations key = k item = nomination}
                {foreach from = $nomination.nominations key = sk item = seconder}
                        ...
                        {if $seconder.is_eligible_candidate == 1 and $seconder.has_accepted_nomination == 1 and $seconder.has_rejected_nomination == 0 and $loggedInContactId == $seconder.member_nominee}
                            <div class="messages status no-popup">
                                <span class="eligible-block">You have accepted the nomination for {$nomination.name}</span> <input type="button" value="{ts}Withdraw Nomination{/ts}" onclick="window.location.href='{crmURL p="civicrm/elections/nominations/reject" q="`$checksum_query['query']`&enid=`$seconder.id`"}'" class="election-action-button" />
                            </div>
                        {/if}
                        {if $election->isNominationsInProgress and $seconder.is_eligible_candidate == 0 and $seconder.has_rejected_nomination == 0 and $loggedInContactId == $seconder.member_nominee}
                            <div class="messages status no-popup">
                                <span class="eligible-block">You have been nominated for {$nomination.name}</span> <input type="button" value="{ts}Withdraw Nomination{/ts}" onclick="window.location.href='{crmURL p="civicrm/elections/nominations/reject" q="`$checksum_query['query']`&enid=`$seconder.id`"}'" class="election-action-button" />
                            </div>
                        {/if}
                {/foreach}
          {/foreach}
