{if $isElectionAdmin and (!$isElectionRunning or !$election->is_visible)}
    <a href="{crmURL p="civicrm/elections/positions/create" q="eid=`$election->id`"}" class="button election-nav-action-button"><span><i class="crm-i fa-plus-circle"></i> {ts}Add Position{/ts}</span></a>
