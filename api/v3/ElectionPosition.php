...
function civicrm_api3_election_position_create($params) {
  ...
  if (!isset($params['result_status']) || $params['result_status'] == '') {
    if ($election->isNominationsStarted && $election->is_visible == 1) {
      return civicrm_api3_create_error ( E::ts('Election position cannot be added/modified once election has been started.'));
    }
    if ($election->is_deleted) {
      return civicrm_api3_create_error ( E::ts('Election position cannot be added/modified for deleted election.'));
    }
  }
...
function civicrm_api3_election_position_delete($params) {
  ...
  $election = findElectionById($electionId, FALSE);
  if ($election->isNominationsStarted && $election->is_visible == 1) {
    return civicrm_api3_create_error ( E::ts('Election position cannot be deleted once election has been started.'));
  }
