...
function civicrm_api3_election_nomination_create($params) {
  if (!isset($params['id'])) {
    ...
    if ($nominationsCount > 0) {
      return civicrm_api3_create_error ( E::ts('Member is already nominated for the selected position.'));
    }
    ...
    if ($election->is_deleted || !$election->is_visible) {
      return civicrm_api3_create_error ( E::ts('Nomination cannot be added for deleted election.'));
    }

    if (!$election->isNominationsStarted) {
      return civicrm_api3_create_error ( E::ts('Nomination cannot be added before nomination period is started.'));
    }

    if (!$election->isNominationsInProgress) {
      return civicrm_api3_create_error ( E::ts('Nomination cannot be added after nomination period is ended.'));
    }
