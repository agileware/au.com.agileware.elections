...
function civicrm_api3_election_vote_deletevotes($params) {
  ...
  if (is_array($params['member_id'])) {
    return civicrm_api3_create_error ( E::ts('Multiple members are not yet supported.'));
  }
...
function civicrm_api3_election_vote_addvotes($params) {
  ...
  if ($election->is_deleted || !$election->is_visible) {
    return civicrm_api3_create_error ( E::ts('Votes cannot be added for deleted election.'));
  }

  if (!$election->isVotingStarted) {
    return civicrm_api3_create_error ( E::ts('Votes cannot be added before voting period is started.'));
  }

  if ($election->isVotingEnded) {
    return civicrm_api3_create_error ( E::ts('Votes cannot be added after voting period is ended.'));
  }
  ...
  if (hasLoggedInUserAlreadyVoted($election->id, $params['member_id'])) {
    if (!$election->allow_revote) {
      return civicrm_api3_create_error ( E::ts('Member has already voted in this election.'));
    }
    else {
  ...
  if (!isLoggedInMemberAllowedToVote($election->id, $params['member_id'])) {
    return civicrm_api3_create_error ( E::ts('Member is not allowed to vote in this election.'));
  }
  ...
  return civicrm_api3_create_success(array(
    'message' => E::ts('%1 has been added successfully.', array(1 => count($votes))),
