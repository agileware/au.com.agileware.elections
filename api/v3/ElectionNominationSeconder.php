...
function civicrm_api3_election_nomination_seconder_create($params) {
  if (!isset($params['id'])) {
    ...
    if ($nominationSeconderCount > 0) {
      return civicrm_api3_create_error ( E::ts('Member is already nominated by given nominator.'));
    }
    ...
    if ($election->is_deleted || !$election->is_visible) {
      return civicrm_api3_create_error ( E::ts('Nomination Seconder cannot be added for a deleted election.'));
    }

    if (!$election->isNominationsInProgress) {
      return civicrm_api3_create_error ( E::ts('Nomination Seconder cannot be added after nomination period is ended.'));
    }
  }
